...
var chatboxManager = function() {
	...
	messageSent : function(dest, msg) {
	    // override this
	    jQuery("#" + dest).chatbox("option", "boxManager").addMsg(dest, msg);
	}
    };
	...
	if(idx != -1) {
		...
		offset = jQuery("#" + showList[i]).chatbox("option", "offset");
		jQuery("#" + showList[i]).chatbox("option", "offset", offset - diff);
	    }
	}
	else {
	...
	if(idx1 != -1) {
	    ...
	    jQuery("#"+id).chatbox("option", "offset", getNextOffset());
	    var manager = jQuery("#"+id).chatbox("option", "boxManager");
	    manager.toggleBox();
	    showList.push(id);
	}
	else{
	...
	jQuery("#" + id).chatbox("option", "boxManager").addMsg(user.first_name, msg);
    }
